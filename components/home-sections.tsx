...
export function FreeExamPrepSection() {
  return <section className="section container free-exam-prep"><div className="section-heading"><div><SectionBadge>Free exam preparation</SectionBadge><h2>Start with reviewed course coverage and a free diagnostic</h2></div><Link href="/exam-prep">View all exam-prep courses -&gt;</Link></div><p className="free-exam-prep-intro">Create a free account to save your semester courses and scores. Paid practice remains locked until the reviewed banks and payment gate are ready.</p><div className="platform-public-grid">{examPrepCourses.slice(0, 3).map((course) => <article key={course.code}><span>{course.level} level · Semester {course.semester}</span><h3 className="course-card-heading"><span>{course.code}</span>{course.title}</h3><p>{course.description}</p><Link href={`/exam-prep/${course.slug}`}>Review course coverage</Link></article>)}</div><div className="free-exam-prep-actions"><Link className="button" href="/account/sign-up">Create account for free diagnostic</Link><Link href="/account/sign-in?next=/dashboard/practice">Already registered? Sign in</Link></div></section>;
}
