...
export function Footer() {
  ...
  return <footer className="footer"><div className="container footer-grid"><div><Link className="logo logo-light" href="/"><BrandLogo variant="wordmark" tone="light" className="brand-logo-footer" /></Link><p>{site.tagline}. Straightforward help for NOUN students.</p><p className="footer-independence">NounCompass is independent. It is not part of NOUN and cannot act for the university.</p><a href={EDITORIAL_PROFILE_URL}>Meet the editorial team</a></div><div><h2>Student help</h2>{navItems.slice(0, 6).map((item) => <Link key={item.href} href={item.href}>{item.label}</Link>)}<Link href="/exam-prep">Exam preparation</Link><Link href="/membership">Semester pass</Link><Link href="/dashboard">Student dashboard</Link></div><div><h2>Your account</h2><Link href="/account/sign-in">Sign in</Link><Link href="/account/sign-up">Create free account</Link><Link href="/dashboard">Continue in dashboard</Link></div><div><h2>Trust & legal</h2>{trustLinks.map(([label, href]) => <Link key={href} href={href}>{label}</Link>)}</div></div><div className="container footer-social-wrap"><SocialLinks compact title="Follow NounCompass on social media" intro="Use the official NounCompass social pages for quick reminders, short updates, and site news." /></div><div className="container footer-bottom"><p>{site.disclaimer}</p><div className="footer-meta"><p>Copyright {new Date().getFullYear()} NOUN Compass</p><a className="footer-credit" href="https://webgrowth.info" target="_blank" rel="noopener noreferrer" aria-label="Built and managed by WebGrowth"><span className="footer-credit-label">Built and managed by</span><Image src="/images/brand/web-growth-logo.webp" alt="WebGrowth" width={220} height={48} className="footer-credit-logo" /></a></div></div></footer>;
}
