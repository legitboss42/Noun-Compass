...
export function NewsletterForm() {
  ...
  return <form className="newsletter-form" onSubmit={submit} noValidate><div className="newsletter-fields"><label className="sr-only" htmlFor="newsletter-email">Email address</label><input id="newsletter-email" name="email" type="email" autoComplete="email" placeholder="you@example.com" required /><button type="submit" disabled={submitting}>{submitting ? "Saving..." : "Join the free list"}</button></div><label className="newsletter-consent"><input name="consent" type="checkbox" required />I agree to receive occasional NounCompass emails about new guides and study updates. I can unsubscribe at any time.</label><label className="newsletter-honeypot" aria-hidden="true">Website<input name="website" tabIndex={-1} autoComplete="off" /></label><p className={error ? "newsletter-message newsletter-message-error" : "newsletter-message"} role="status" aria-live="polite">{message}</p><small>No paid membership is needed. Read the <Link href="/privacy-policy">Privacy Policy</Link>.</small></form>;
}
