...
type ReviewItem = { questionVersionId: string; prompt: string; explanation: string; correct: boolean; selectedAnswer: string; correctAnswer: string };
...
export function PracticeRunner({ banks, premium, dueCount }: { banks: Array<{ course_code: string; course_title: string }>; premium: boolean; dueCount: number }) {
  ...
  return <section className="platform-panel"><h2>Start a practice session</h2>{result && <><p className="form-message form-message-success">Session complete: {result.score}% ({result.correct}/{result.total} correct; {result.answered} answered).</p>{result.review?.length ? <details><summary>Review answers and explanations</summary><div className="platform-ticket-list">{result.review.map((item, reviewIndex) => <article key={item.questionVersionId}><strong>{reviewIndex + 1}. {item.prompt}</strong><span>{item.correct ? "Correct" : "Needs review"}</span><p><strong>Your answer:</strong> {item.selectedAnswer}</p><p><strong>Correct answer:</strong> {item.correctAnswer}</p><p>{item.explanation}</p></article>)}</div></details> : null}</>}
    <div className="platform-form-grid"><label>Course<select value={courseCode} onChange={(event) => setCourseCode(event.target.value)}>{banks.map((bank) => <option key={bank.course_code} value={bank.course_code}>{bank.course_code} — {bank.course_title}</option>)}</select></label><label>Mode<select value={mode} onChange={(event) => setMode(event.target.value as Mode)}><option value="diagnostic">Free diagnostic</option><option value="practice" disabled={!premium}>Premium practice</option><option value="revision" disabled={!premium || dueCount === 0}>Due revision ({dueCount})</option><option value="timed-mock" disabled={!premium}>40-minute timed mock</option></select></label></div>{status && <p className="form-message form-message-error" role="status">{status}</p>}<button className="button" type="button" onClick={start}>Start session</button></section>;
}
