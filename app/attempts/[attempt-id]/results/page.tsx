...
import {
  ...
  StudentMobileHeader,
} from "@/components/student-mobile-navigation";
import { requireUser } from "@/lib/platform/auth";
import {
  createQuestionStore,
  StoreError,
} from "@/lib/platform/question-store";
...
export default async function AttemptResultsPage({
  ...
  params: Promise<{ "attempt-id": string }>;
}) {
  const user = await requireUser("/dashboard/practice");
  let result;
  try {
    result = await createQuestionStore().complete(
      user.id,
      (await params)["attempt-id"],
    );
  } catch (error) {
  ...
  return (
    <main
      id="main-content"
      className="platform-shell attempt-results-shell"
      data-auth-shell="student"
    >
      <StudentMobileHeader
      ...
      <div className="container narrow attempt-results-content">
        <Link className="attempt-back-link" href="/dashboard/practice">
          ← Back to practice
        </Link>
        <section className="platform-heading attempt-score-card">
          <div>
            ...
            <h1>{result.score}%</h1>
            <p>
              {result.correct} correct from {result.total} questions;{" "}
              {result.answered} answered.
            </p>
          </div>
          <Link className="button" href="/dashboard/practice">
