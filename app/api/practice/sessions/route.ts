...
export async function POST(request: Request) {
  ...
  if (!courseCode || !["diagnostic", "practice", "timed-mock"].includes(mode ?? "")) return NextResponse.json({ message: "Invalid practice request." }, { status: 400 });
  ...
  if (mode !== "diagnostic" && !premium) return NextResponse.json({ message: "A current semester pass is required for full practice and timed mocks." }, { status: 403 });
  ...
  const { data: questions, error: questionError } = await questionQuery.limit(limit);
  ...
  const { data: session, error: sessionError } = await admin.from("practice_sessions").insert({ user_id: user.id, bank_id: bank.id, mode, question_count: currentVersions.length }).select("id,mode,started_at").single();
  ...
  return NextResponse.json({
    session: { ...session, courseCode: bank.course_code, courseTitle: bank.course_title },
