...
## Final review fix wave (2026-08-12)

- Stage-aware re-engagement delivery now preflights `UNSUBSCRIBE_SECRET` before the notification delivery adapter is invoked. `StageEmailConfigurationError` carries the explicit safe code `missing_unsubscribe_secret` and states that no notifications were created; the admin action's existing error redirect surfaces that message without selecting a success result. The extracted non-server delivery core makes the ordering testable and preserves the server-only wrapper for actual email access.
- TDD evidence: the new fake-notification-database test initially could not find the extracted safety/delivery core; after implementation it proves the missing-secret path rejects before `insertNotification` is called. Focused notification/re-engagement verification passed with 20 tests and 0 failures; `npx tsc --noEmit` and `git diff --check` passed.
- `docs/platform-operations.md` and the historical portion of `docs/credential-register.md` now label prior Flutterwave setup, credential, webhook, sandbox/live initialization, deployment, and checkout-release statements as historical records requiring owner-authorized re-verification. Current guidance remains the unverified provider transaction/callback/webhook/production-row status in `docs/monetization-operations.md`.
